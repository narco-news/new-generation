...
<template>
  <section class="box__wrapper">
    <div ref="boxHeadingBar" class="box__heading-bar">
      <h1 class="box__heading">
        {{ props.heading }}
      </h1>
      <router-link :to="`/tags/${props.slug}`" class="box__more-link">
        READ MORE
        &nbsp;
        <svg
          width="15"
    ...
    <div class="box__articles-wrapper">
      <div v-for="article in props.articles" :key="article.slug" class="box__article">
        <latest-box-article :article="article" :show-image="props.showImage" />
      </div>
    </div>
  </section>
</template>
